...
char string[20];
typedef struct hospital akshay;
...
{
while(fread(&p,sizeof(akshay),1,fp))
s=1+p.sn;
}
fclose(fp);
}
...
while(fread(&p,sizeof(akshay),1,fp))
if(p.sn!=y)
fwrite(&p,sizeof(akshay),1,f1);
clrscr();
mainscreen();
fseek(fp,(y-1)*sizeof(akshay),SEEK_SET);
fread(&p,sizeof(akshay),1,fp);
print();
edit1();
gotoxy(25,46);
...
fseek(fp,0,SEEK_END);
tsz=ftell(fp);
n=(int)(tsz/sizeof(akshay));
for(i=0;i<(n-1);i++)
{
for(a=i+1;a<n;a++)
{
fseek(fp,i*sizeof(akshay),SEEK_SET);
fread(&p,sizeof(akshay),1,fp);
fseek(fp,a*sizeof(akshay),SEEK_SET);
fread(&q,sizeof(akshay),1,fp);
if(strcmp(p.name,q.name)>0)
{
fseek(fp,i*sizeof(akshay),SEEK_SET);
fwrite(&q,sizeof(akshay),1,fp);
fseek(fp,a*sizeof(akshay),SEEK_SET); fflush(stdin);
fwrite(&p,sizeof(akshay),1,fp);
}
}
}
rewind(fp);
